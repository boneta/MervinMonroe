subroutine gamess_server( code, natm, naqm, atmn, cord, ener, grad, hess )
	...
	write( 999, "(a)" ) " $ecp"
	do i = 1, naqm
		open( unit = 998, file = trim( smb(int(atmn(i))) ) // ".ecp", action = "read", form = "formatted" )
		read( 998, * ) k
		do j = 1, k
			read( 998, "(a)" ) str
			write( 999, "(a)" ) trim( str )
		end do
		close( 998 )
	end do
	write( 999, "(a)" ) " $ecp"
	...
	do i = 1, naqm
		...
		open( unit = 998, file = trim( smb(int(atmn(i))) ) // ".basis", action = "read", form = "formatted" )
		read( 998, * ) k
		do j = 1, k
			read( 998, "(a)" ) str
			write( 999, "(a)" ) trim( str )
