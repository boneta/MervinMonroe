subroutine chrg_server( code, natm, naqm, atmn, cord, ener, qfit, grad, hess )
	...
	write( 999, "(a/a/a/a)" ) "%Chk=calc", &
		"%Nproc=1", &
		"%Mem=512mb", &
		"#t b3lyp/6-31g(d,p) charge guess=(read,tcheck) scf=(direct,conver=6) nosymm"
	if( code == 1 ) then
			write( 999, "(a/)" ) "force pop=chelpg fchk"
	else if( code == 2 ) then
			open( unit=998, file = "guess", action = "read", form = "unformatted", iostat = ierr )
			close( 998 )
			if( ierr == 0 ) then
!
!-- sometimes POP calculation after the OPT one abnormaly stops gaussian
!
				write( 999, "(a/)" ) "opt=(maxcyc=1,maxstep=1) pop=chelpg fchk"
				i = system( "/bin/rm -f guess" )
			else
	...
	i = system( ". /usr/local/Chem/g03d02/bin/g03.profile; g03 calc" )
