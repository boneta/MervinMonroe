subroutine chrg_server( code, natm, naqm, atmn, cord, ener, qfit, grad, hess )
	...
	write( 999, "(a/a/a/a)" ) "%Chk=calc", &
		"%Nproc=8", &
		"%Mem=8000mb", &
		"#t utpsstpss/gen pseudo=lanl2 iop(3/76=0900001000) charge guess=tcheck scf=(direct,conver=5,maxcyc=500) nosymm"
	if( code == 1 ) then
			write( 999, "(a/)" ) "force pop=(mk,readradii) fchk"
	else if( code == 2 ) then
			write( 999, "(a/)" ) "force pop=(mk,readradii) fchk"
	else
			write( 999, "(a/)" ) "pop=(mk,readradii) fchk"
	end if
	write( 999, "(a//2i4)" ) "- slave -", -2, 4
	do i = 1, naqm
		j = ( i - 1 ) * 3
		write( 999, "(a8,3f20.10)" ) smb(int(atmn(i))), cord(j+1), cord(j+2), cord(j+3)
	end do
	write( 999, "(/a/a/a)" ) "Fe 0", "Lanl2dz", "****"
	write( 999, "(a/a/a/)" ) "N,C,H,O,S 0", "6-31G*", "****"
	smm = 0.0d0
	if( naqm == natm ) then
	...
	i = system( ". /scratch1/smarti/g03/g03.profile; g03 calc" )
...
999	continue
	...
	if( code == 2 ) then
		open( unit = 888, file = "hessian.fchk", action = "read", form = "formatted" )
		read( 888, "(a)", end = 888 ) str
		do while( .true. )
			if( str(1:25) == "Cartesian Force Constants" ) then
