...
#ifdef	USE_XOR
                IF( XOR( PERT_SELE(QATOM), PERT_SELE( MATOM ) ) ) THEN
#else
                IF( ( PERT_SELE(QATOM) .AND. .NOT. PERT_SELE(MATOM) ) .OR. ( .NOT. PERT_SELE(QATOM) .AND. PERT_SELE(MATOM) ) ) THEN
#endif
                    IF( PERT_CALC_LJ ) THEN
                        CGL = .0_DP
                    ELSE
                        CGL = ATMCHG(MATOM) * LAMBDA_VALUE
                    END IF
                END IF
            END IF
#endif
