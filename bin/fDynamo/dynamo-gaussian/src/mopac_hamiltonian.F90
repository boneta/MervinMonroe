...
USE ATOMS,            ONLY : ATMMAS, ATMNUM, ATMQMI, NATOMS, NATOMSMM, NATOMSQM, atmnam
USE GAUSSIAN_BASIS
...
write( output, "(80('-'))" )
write( output, "(5a10)" ) "iatom", "label", "atmnum", "core(ni)", "sum el." 
   ! . Loop over the atoms.
   DO IATOM = 1,NATOMS
      ...
      NUMORB = NATORB(NI)
write( output, "(i10,a10,3i10)" ) iatom, trim( atmnam( iatom ) ), ni, nint( core(ni) ), nint( elecs )
...
write( output, "(80('-'))" )
	 ...
	 DO IBOND = 1,NBONDS
	       ...
	       QATMNUM(J) = -I
write( output, "(a)" ) "QM:[" // trim( atmnam(i) ) // "] --- LA:(" // trim( atmnam(j) ) // ")"
	    ELSE IF ( QMFLAG(J) .AND. ( .NOT. QMFLAG(I) ) ) THEN
               NBOUNDARY  = NBOUNDARY + 1
	       QATMNUM(I) = -J
write( output, "(a)" ) "QM:[" // trim( atmnam(j) ) // "] --- LA:(" // trim( atmnam(i) ) // ")"
            END IF
