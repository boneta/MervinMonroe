...
PUBLIC :: CUT_OFF, CUT_ON, ENERGY_NON_BONDING_CALCULATE, ENERGY_NON_BONDING_INTERACTION,      &
          ...
          NBLIST_TYPE, NBLISTQM_FIRST, NNBLISTQM, NQMINT, QIMAGE, UPDATE,                     &
   ...
   DO IREC = 1,NRECORDS
      ...
      DO IINT = 1,SIZE(NBCURRENT%INTERACTIONS)
         ...
         IF ( RIJ2 <= R2ON ) THEN
            ...
            IF( ( ATMQMI(I) == 0 .AND. ATMQMI(J) == 0 ) .OR. SKIP_CABINITIO ) &
                EELECT = EELECT + ETEMP + QIJ * SHIFT_EL1
#else

#ifdef	HACK_PERT
            EL0 = ETEMP + QIJ * SHIFT_EL1
            IF( PERT_CALC_EL .OR. PERT_CALC_LJ ) THEN
#ifdef	USE_XOR
                IF( XOR( PERT_SELE(I), PERT_SELE(J) ) ) THEN
#else
                IF( ( PERT_SELE(I) .AND. .NOT. PERT_SELE(J) ) .OR. ( .NOT. PERT_SELE(I) .AND. PERT_SELE(J) ) ) THEN
#endif
                    IF( PERT_CALC_LJ ) THEN
                        EL0 = .0_DP
                    ELSE
                        CUR_PERT_EL = CUR_PERT_EL + LAMBDA_VALUE * EL0
                        IF( LAMBDA_VALUE + LAMBDA_DELTA > 1._DP .OR. LAMBDA_DELTA == .0_DP ) THEN
                            NXT_PERT_EL = NXT_PERT_EL + LAMBDA_VALUE * EL0
                        ELSE
                            NXT_PERT_EL = NXT_PERT_EL + ( LAMBDA_VALUE + LAMBDA_DELTA ) * EL0
                        END IF
                        IF( LAMBDA_VALUE - LAMBDA_DELTA < 0._DP .OR. LAMBDA_DELTA == .0_DP ) THEN
                            PRV_PERT_EL = PRV_PERT_EL + LAMBDA_VALUE * EL0
                        ELSE
                            PRV_PERT_EL = PRV_PERT_EL + ( LAMBDA_VALUE - LAMBDA_DELTA ) * EL0
                        END IF
                        EL0 = EL0 * LAMBDA_VALUE
                    END IF
                END IF
            END IF
            EELECT = EELECT + EL0
            ...
            IF( PERT_CALC_LJ ) THEN
                ...
                IF( ( PERT_SELE(I) .AND. .NOT. PERT_SELE(J) ) .OR. ( .NOT. PERT_SELE(I) .AND. PERT_SELE(J) ) ) THEN
#endif
                    CUR_PERT_LJ = CUR_PERT_LJ + LAMBDA_VALUE * TMP_INT
                    IF( LAMBDA_VALUE + LAMBDA_DELTA > 1._DP .OR. LAMBDA_DELTA == .0_DP ) THEN
                        NXT_PERT_LJ = NXT_PERT_LJ + LAMBDA_VALUE * TMP_INT
                    ELSE
                        NXT_PERT_LJ = NXT_PERT_LJ + ( LAMBDA_VALUE + LAMBDA_DELTA ) * TMP_INT
                    END IF
                    IF( LAMBDA_VALUE - LAMBDA_DELTA < 0._DP .OR. LAMBDA_DELTA == .0_DP ) THEN
                        PRV_PERT_LJ = PRV_PERT_LJ + LAMBDA_VALUE * TMP_INT
                    ELSE
                        PRV_PERT_LJ = PRV_PERT_LJ + ( LAMBDA_VALUE - LAMBDA_DELTA ) * TMP_INT
                    END IF
                    TMP_INT = LAMBDA_VALUE * TMP_INT
                END IF
            END IF
#endif
            ELJ = ELJ + TMP_INT
            ...
            IF( PERT_CALC_EL .OR. PERT_CALC_LJ ) THEN
                ...
                IF( ( PERT_SELE(I) .AND. .NOT. PERT_SELE(J) ) .OR. ( .NOT. PERT_SELE(I) .AND. PERT_SELE(J) ) ) THEN
#endif
                    IF( .NOT. PERT_CALC_LJ ) DF = DF - LAMBDA_VALUE * ETEMP / RIJ2
                ELSE
                    DF = DF - ETEMP / RIJ2
                END IF
            ELSE
            ...
            IF( PERT_CALC_LJ ) THEN
                ...
                IF( ( PERT_SELE(I) .AND. .NOT. PERT_SELE(J) ) .OR. ( .NOT. PERT_SELE(I) .AND. PERT_SELE(J) ) ) THEN
#endif
                    DF = DF + LAMBDA_VALUE * 6.0_DP * EIJ * ( S6 - 2.0_DP * S12 ) / RIJ2
                ELSE
                    DF = DF + 6.0_DP * EIJ * ( S6 - 2.0_DP * S12 ) / RIJ2
                END IF
            ELSE
                DF = DF + 6.0_DP * EIJ * ( S6 - 2.0_DP * S12 ) / RIJ2
            END IF
         ...
         ELSE
...
#else
#ifdef	HACK_PERT
            EL0 = QIJ * ( A * S - B * R - C * R3 - D * R5 + SHIFT_EL2 )
            IF( PERT_CALC_EL .OR. PERT_CALC_LJ ) THEN
#ifdef	USE_XOR
                IF( XOR( PERT_SELE(I), PERT_SELE(J) ) ) THEN
#else
...
#endif
                    IF( .NOT. PERT_CALC_LJ ) &
                    DF = DF - LAMBDA_VALUE * QIJ * ( A / R3 + B * S + 3.0_DP * C * R + 5.0_DP * D * R3 ) 
                ELSE
                    DF = DF - QIJ * ( A / R3 + B * S + 3.0_DP * C * R + 5.0_DP * D * R3 ) 
                END IF
            ELSE
                DF = DF - QIJ * ( A / R3 + B * S + 3.0_DP * C * R + 5.0_DP * D * R3 ) 
            END IF
            IF( PERT_CALC_LJ ) THEN
                ...
                IF( ( PERT_SELE(I) .AND. .NOT. PERT_SELE(J) ) .OR. ( .NOT. PERT_SELE(I) .AND. PERT_SELE(J) ) ) THEN
#endif
                    DF = DF - LAMBDA_VALUE * 6.0_DP * EIJ * ( 2.0_DP * K12 * S6 * ( S6 - SHIFT_LJ12 ) - &
                              K6 * S3 * ( S3 - SHIFT_LJ6 ) ) / RIJ2
                ELSE
                    DF = DF - 6.0_DP * EIJ * ( 2.0_DP * K12 * S6 * ( S6 - SHIFT_LJ12 ) - K6 * S3 * ( S3 - SHIFT_LJ6 ) ) / RIJ2
                END IF
            ELSE
                DF = DF - 6.0_DP * EIJ * ( 2.0_DP * K12 * S6 * ( S6 - SHIFT_LJ12 ) - K6 * S3 * ( S3 - SHIFT_LJ6 ) ) / RIJ2
            END IF
