...
PRIVATE
PUBLIC :: CABINITIO_INIT, CABINITIO_SETUP, CABINITIO_ENERGY, CABINITIO_GRADIENT, CABINITIO_HESSIAN, CABINITIO_EXIT

CONTAINS


SUBROUTINE CABINITIO_NON_BONDED( SELECTION )
    IMPLICIT NONE
...
!	WHERE( ATMQMI > 0 ) SELECTION = .FALSE.
    ...
    ! - skip QM-atoms (keep MM-Link) from the interaction...
	DO I = 1, NATOMSQM
		IF( .NOT. QMATOM(I)%QBOUNDARY ) SELECTION(QMATOM(I)%ATOM) = .FALSE.
	END DO
END SUBROUTINE
...
SUBROUTINE CABINITIO_REMOVE_MM_TERMS( SELECTION )
    IMPLICIT NONE
...
SUBROUTINE CABINITIO_SETUP( SELECTION )
    IMPLICIT NONE
    LOGICAL, DIMENSION(1:NATOMS), INTENT(IN) :: SELECTION
    ...
    DO I = 1, NATOMS
        IF( IDX(I) > 0 ) THEN
            ...
            K1                   = K1 + 1
write(*,*) "QM:", idx(i), atmqmi(i), atmnum(i), trim(atmnam(i))
        END IF
        IF( IDX(I) < 0 ) THEN
            ATMQMI(I)            = K2
            QMATOM(K2)%ATOM      = I
            ...
            QMATOM(K2)%BLAST     = 0
            QMATOM(K2)%COPY      = 0
			! - non Interacting Link-Atoms
            QMATOM(K2)%NUMBER    = -1
			! - Interacting Link-Atoms
!            QMATOM(K2)%NUMBER    = 1
            QMATOM(K2)%LENGTH    = 1._DP
            QMATOM(K2)%PARTNER   = - IDX(I)
            QMATOM(K2)%QBOUNDARY = .TRUE.
            K2                   = K2 - 1
write(*,*) "LA:", idx(i), atmqmi(i), atmnum(i), trim(atmnam(i))
        END IF
    END DO
    DEALLOCATE( IDX )
    CALL CABINITIO_REMOVE_MM_TERMS( SELECTION )
     QUSEBAQM      = .FALSE.
    SKIP_CABINITIO = .FALSE.
END SUBROUTINE
...
SUBROUTINE CABINITIO_ENERGY( ENERGY )
    IMPLICIT NONE
    ...
    CALL CABINITIO_NON_BONDED( SELE )
...
SUBROUTINE CABINITIO_GRADIENT( ENERGY, GRADIENT )
    IMPLICIT NONE
...
SUBROUTINE CABINITIO_HESSIAN( ENERGY, GRADIENT, HESSIAN )
    IMPLICIT NONE
