...
!write(*,*) dihedrals(i)%i, dihedrals(i)%j, dihedrals(i)%k, dihedrals(i)%l, &
!			dihedrals(i)%fc, dihedrals(i)%period, dihedrals(i)%phase
   END DO
...
#ifdef	HACK_AMBER
            ! . V4.
            IF ( DIHEPS(MM)%V4 /= 0.0_DP ) THEN
               N = N + 1
               WRITE ( TUNIT ) D1, D2, D3, D4, DIHEPS(MM)%V4, 4,  -1.0_DP
            END IF
#endif
         END IF
...
#ifdef	HACK_AMBER
            ! . V4.
            IF ( IMPRPS(MM)%V4 /= 0.0_DP ) THEN
               N = N + 1
               WRITE ( TUNIT ) I1, I2, I3, I4, IMPRPS(MM)%V4, 4,  -1.0_DP
            END IF
#endif
         END IF
