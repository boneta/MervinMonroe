...
! - mopac excessive iterations xyz flush -----------------------------------------------------------------------------
!                              qmatom, mopac_data_atom_position
!use atoms,             only : atmcrd, natomsqm
    ...
    END IF
	 ...
	 IF ( CVGED ) EXIT
      ...
      IF ( .NOT. CVGED ) THEN
          IF ( QSCFBOMB ) THEN
! - mopac excessive iterations xyz flush -----------------------------------------------------------------------------
call coordinates_write( "mopac_scf_error.crd" )
!open(unit=999,file="mopac_scf_error.xyz",action="write",form="formatted")
!write(999,"(i6/)") natomsqm
!do iter = 1, natomsqm
!write(999,"(a2,3f20.10)") symbol(qmatom(iter)%number), mopac_data_atom_position( iter, atmcrd )
!end do
!close(999)
! --------------------------------------------------------------------------------------------------------------------
