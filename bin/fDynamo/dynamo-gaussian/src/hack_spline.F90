...
! ---------------------------------------------------------------------------------------------------------
! - Esto corresponde a un par de coordenadas: antisimétrica (A -- B -- C) [X] vs distancia (C -- D) [Y]
   CASE ( "PACHARAN" )
       NCOEF = 1
       QOK = .TRUE.
       NSPOINTS = 4
       CALL PRINT_PARAGRAPH( TEXT = "[PACHARAN]: (A -- B -- C) [X] vs (C -- D) [Y]" )
! ---------------------------------------------------------------------------------------------------------
! - Esto corresponde a un par de coordenadas: antisimétrica (A -- B -- C) [X] vs distancia (A -- D) [Y]
   CASE ( "XPACHARAN" )
       NCOEF = 1
       QOK = .TRUE.
       NSPOINTS = 4
       CALL PRINT_PARAGRAPH( TEXT = "[XPACHARAN]: (A -- B -- C) [X] vs (A -- D) [Y]" )
! ---------------------------------------------------------------------------------------------------------
! - Esto corresponde a un par de coordenadas: antisimétrica (A -- B -- C) [X] vs distancia (D -- E) [Y]
   CASE ( "HOMEBREW" )
       ...
       QOK = .TRUE.
       NSPOINTS = 5
       CALL PRINT_PARAGRAPH( TEXT = "[HOMEBREW]: (A -- B -- C) [X] vs (D -- E) [Y]" )
! ---------------------------------------------------------------------------------------------------------
! ---------------------------------------------------------------------------------------------------------
! - Esto corresponde a un par de coordenadas antisimétrica (A -- B -- C) [X] vs (D -- E -- F) [Y]
   CASE ( "BURRITO_LOCO" )
       ...
       QOK = .TRUE.
       NSPOINTS = 6
       CALL PRINT_PARAGRAPH( TEXT = "[BURRITO_LOCO]: (A -- B -- C) [X] vs (D -- E -- F) [Y]" )
! ---------------------------------------------------------------------------------------------------------
...
! --------------------------------------------------------------------------------------
      CASE ( "PACHARAN" ) ; CALL PACHARAN
      CASE ( "XPACHARAN" ) ; CALL XPACHARAN
...
! --------------------------------------------------------------------------------------
      ...
      DR(3,1:3) = CENTERS(1:3,3) - CENTERS(1:3,4)
...
!!write(*,'(a,6f12.6)') 'S2D', value(1), value(2), e0(1), e0(2), df(1), df(2)
      ...
      DF(2)       =   DF(2) / VALUE(2)
      DEDC(1:3,3) = DEDC(1:3,3) +  DF(2) * DR(3,1:3)
      DEDC(1:3,4) = - DF(2) * DR(3,1:3)

      END SUBROUTINE PACHARAN
! --------------------------------------------------------------------------------------

! --------------------------------------------------------------------------------------
      SUBROUTINE XPACHARAN
      ...
      DR(3,1:3) = CENTERS(1:3,1) - CENTERS(1:3,4)
      VALUE(2)  = SQRT ( DOT_PRODUCT ( DR(3,1:3), DR(3,1:3) ) )
...
!!write(*,'(a,6f12.6)') 'S2D', value(1), value(2), e0(1), e0(2), df(1), df(2)
      ...
      DF(2)       =   DF(2) / VALUE(2)
      DEDC(1:3,1) = DEDC(1:3,1) +  DF(2) * DR(3,1:3)
      DEDC(1:3,4) = - DF(2) * DR(3,1:3)

      END SUBROUTINE XPACHARAN
! --------------------------------------------------------------------------------------
